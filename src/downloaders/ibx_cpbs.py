...
def scrape_medical_policies(homepage_url: str) -> list[dict]:
    options = Options()
...
def scrape_pharmacy_policies(homepage_url: str) -> list[dict]:
    options = Options()
...
if __name__ == "__main__":
    ...
    targets_list = scrape_medical_policies(homepage)
    ...
    # Scrape target links and compile in jsonl (Pharmacy)
    homepage = "https://medpolicy.ibx.com/ibc/pps/Pages/PolicyBulletinsView.aspx"
    targets_list = scrape_pharmacy_policies(homepage)
    targets_meta_path = os.path.join(download_dir, "pharmacy_ibx_targets.jsonl")
    for target in targets_list:
        add_jsonl_line(targets_meta_path, target)
