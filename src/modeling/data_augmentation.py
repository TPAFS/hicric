...
def rewrite_to_generic(
    ...
    print(f"Starting to rewrite {len(sufficient_examples)} examples with {num_augmentations_per_example} augmentations each ({total_examples} total)")
...
def augment_sufficient_examples(
    ...
    # Print start message
    total_examples = len(sufficient_examples) * num_augmentations_per_example
    print(f"Starting to augment {len(sufficient_examples)} examples with {num_augmentations_per_example} augmentations each ({total_examples} total)")
    ...
    perspective_prompt = "Rewrite the following description from a patient's perspective, maintaining all the key details. {}"
...
def process_and_save_augmentations(
    ...
    dataset_records = [{"text": rec["answers"]["text"][0], "sufficiency_score": rec["sufficiency_score"]} for rec in records]
